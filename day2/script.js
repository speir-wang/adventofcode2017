var input = `278	1689	250	1512	1792	1974	175	1639	235	1635	1690	1947	810	224	928	859
...
/*	--- day 2 part one ---  */
var partOneSum = 0;
inputArray.forEach(function(currentRow, index) {
	var row = currentRow.split("	")
	// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/max
	var max = Math.max(...row) 
	var min = Math.min(...row)
	var diff = max - min
	partOneSum = partOneSum + diff
})
console.log("day 2 part 1 answer: " + partOneSum)

/*	--- day 2 ---  */
var partTwoSum = 0
inputArray.forEach(function(currentRow, index) {
	var row = currentRow.split("	")
	// sort the array from max to min, then use the max to see if it can be evenly divided 
	/*
		improvement in here is to sort the array from min to max, and pop the max from the array,
		because the possibility of having the max evenly divivded by the second biggest number is small,
		
	*/ 
	row = row.sort(compareNumbers)
	row.forEach(function(rowElement, index) {
		rowElement = parseInt(rowElement)
		var tempRow = row.slice(index+1)

		tempRow.forEach(function(tempRowElement, index) {
			if (rowElement % tempRowElement === 0) {
				var div = rowElement / tempRowElement
				partTwoSum = partTwoSum + div
				return 
			}
		})
	})
})
console.log("day 2 part 2 answer: " + partTwoSum)
