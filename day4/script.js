...
// var input = `bkvlfuw onkqzeo iwrg rifqzhj mgroul rnor qqqc sbfi hny zosfp kopxb
// nvifbx jbowbj fnyskt jbowbj xvun xvun
// piyl haajm stwzpp xvjg amjah
// gye efwwwiv kyv zmtcgmi ifwvwew`
...
var repeatedRow = 0
var partOneSum = 0
var totalRows = inputArray.length

var firstRepeatedPass = ''
inputArray.forEach(function(currentRow, index) {
	var row = currentRow.split(" ")
	/*
		The thing learnt in here: continue and break will not be working in Array.forEach
		because the loop in Array.forEach is actually a callback function.

		Improvement: using Array.filter
	*/ 
	for(var i = 0; i < row.length; i ++) {
		var tempRow = row.slice(i+1)
		if ( tempRow.includes(row[i])) {
			firstRepeatedPass = row[i]
			repeatedRow++
			break
		}
	}
...
partOneSum = totalRows - repeatedRow
console.log("part one answer is: " + partOneSum)

/* --- part 2 --- */ 
var partTwoSum = 0
var repeatedSecondRow = 0
inputArray.forEach(function(currentRow, index) {
	var row = currentRow.split(" ")
	for(var i = 0; i < row.length; i ++) {
		row[i] = sortStringAlphabetically(row[i])
	}
...
partTwoSum = totalRows - repeatedSecondRow
