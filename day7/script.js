...
var baseProgram = null
/* --- part 1 --- */
var baseArray = []
var childArray = []
// filter out top layer programs
var filteredArray = partOneArray.filter(function(program, index) {
	if (program.split(' -> ').length == 2) {
		return program
	}
})
filteredArray.forEach(function(element) {
	var firstPart = element.split(' -> ')[0]
	var secondPart = element.split(' -> ')[1]
	baseArray.push(firstPart.split(' ')[0])

	secondPart.split(', ').forEach(function(element) {
		if ( !childArray.includes(element)) {
			childArray.push(element)
		}
	})
})
baseArray.filter(function(element) {
	if(!childArray.includes(element)) {
		baseProgram = element
	}
})
...
/* --- part 2 is not done--- */
function getChildPrograms(parentProgram) {
	var childPrograms = null
	filteredArray.forEach(function(element) {
		// console.log(element)
		if (element.split(' -> ')[0].includes(parentProgram)) {
			childPrograms = element.split(' -> ')[1].split(', ')
		}
	})
...
function getWeight(program) {
	var programWeight = 0;
	partTwoArray.forEach(function(element) {
		if (element.split(' -> ')[0].includes(program)) {
			programWeight = parseInt( element.split(' -> ')[0].split(' ')[1].match(/\(([^)]+)\)/)[1] )
		}
	})
...
function iteration(program) {
	// find its children
	// for each of the root prgrogram children, calculate the total weight of that child tree
	// for the first node, var subtreeTotalWeight
	// add all its
	var childPrograms = null
	if ( !getChildPrograms(program) ) {
		return false
	}
	preOrder(baseProgram)
}

var subtreeTotalWeight = 0
// need to redo preorder function
// 
