...
inputArray.forEach(function(element){
	partOneArray.push(element)
	partTwoArray.push(element)
})
/* ---- part 1 ---- */ 
var partOneLargestRegister = 0
var variablesObjectArray = [] // used to store objects
var variableArray = [] // used to store the temp objects key, just for comparison

partOneArray.forEach(function(element, index){
	var tempArray = element.split(' if ')
	var instruction = tempArray[0]
	var condition = tempArray[1]
	instruction = replaceLiteralInstructionWithSymbol(instruction) // replace the "inc" or "dec" with "+=" or "-="

	// process the condition stuff
	var tempConditionVariable = condition.split(" ")[0]
	var tempConditionObject
	if (!checkIfKeyExists(variablesObjectArray, condition.split(" ")[0])) {
		tempConditionObject = {
			[tempConditionVariable]: 0
		}
		variablesObjectArray.push(tempConditionObject)
	} else {
		var tempValue = findValueFromVariableObject(variablesObjectArray, condition.split(" ")[0])
		tempConditionObject = {
			[tempConditionVariable]: tempValue
		}
	}
	
	// process the instruction stuff
	var tempInstructionVariable = instruction.split(" ")[0]
	var tempInstructionObject
	if (!checkIfKeyExists(variablesObjectArray, instruction.split(" ")[0])) {
		tempInstructionObject = {
			[tempInstructionVariable]: 0
		}
		variablesObjectArray.push(tempInstructionObject)
	} else {
		var tempValue = findValueFromVariableObject(variablesObjectArray, instruction.split(" ")[0])
		tempInstructionObject = {
			[tempInstructionVariable]: tempValue
		}
	}

	if (eval(condition.replace(condition.split(" ")[0], tempConditionObject[tempConditionVariable]))) {
		var tempValue = findValueFromVariableObject(variablesObjectArray, instruction.split(" ")[0])
		eval(`var ${instruction.split(" ")[0]} = ${tempValue}`)
		var newValue = eval(instruction)
		if (newValue > partOneLargestRegister) {
			partOneLargestRegister = newValue
		}
		update(variablesObjectArray, instruction.split(" ")[0], newValue)
	}
})
console.log(partOneLargestRegister)
findLargestNumber(variablesObjectArray)
...
function findLargestNumber(array) {
	...
	array.forEach(function(element, index){
		for (var key in element) {
			if (index === 0) {
				largestNumber = element[key]
			}
			if (element[key] > largestNumber) {
				largestNumber = element[key]	
			}
		}
	})
...
function replaceLiteralInstructionWithSymbol(instructionString) {
	if (instructionString.includes("inc")) {
		instructionString = instructionString.replace("inc", "+=")
	} else {
		instructionString = instructionString.replace("dec", "-=")
	}
	return instructionString
}

function checkIfKeyExists(array, key) {
...
function findValueFromVariableObject(array, key){
	for (var i = 0; i < array.length; i++) {
		if (Object.keys(array[i])[0] === key) {
			return (Object.values(array[i])[0])
		}
	}
	return false
}

function update(array, key, newValue) {
	for (var i = 0; i < array.length; i++) {
		if (Object.keys(array[i])[0] === key) {
